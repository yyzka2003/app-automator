...
Java_com_darpa_YoloV5Ncnn_Init(JNIEnv *env, jobject thiz, jobject assetManager) {
    ncnn::Option opt;
    opt.lightmode = true;
    opt.num_threads = 4;
    ...
    // init jni glue
    jclass localObjCls = env->FindClass("com/darpa/YoloV5Ncnn$Obj");
    objCls = reinterpret_cast<jclass>(env->NewGlobalRef(localObjCls));

    constructortorId = env->GetMethodID(objCls, "<init>", "(Lcom/darpa/YoloV5Ncnn;)V");
...
Java_com_darpa_YoloV5Ncnn_Detect(JNIEnv *env, jobject thiz, jobject bitmap,
                                              jboolean use_gpu) {
    int gpu_count = ncnn::get_gpu_count();

    __android_log_print(ANDROID_LOG_DEBUG, "YoloV5Ncnn", "gpu_count: %d", gpu_count);
    ...
    int w = width;
    int h = height;
    __android_log_print(ANDROID_LOG_DEBUG, "czx yolo w", "%d", w);
    __android_log_print(ANDROID_LOG_DEBUG, "czx yolo h", "%d", h);
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "before", " from_android_bitmap_resize");
    ncnn::Mat in = ncnn::Mat::from_android_bitmap_resize(env, bitmap, ncnn::Mat::PIXEL_RGB, w, h);
    __android_log_print(ANDROID_LOG_DEBUG, "after", " from_android_bitmap_resize");
    ...
    std::vector<Object> objects;
    {
        ...
        {
            ncnn::Mat out;
//            __android_log_print(ANDROID_LOG_DEBUG, "before", " 2.1 ");
            ex.extract("output", out);
//            __android_log_print(ANDROID_LOG_DEBUG, "after", " 2.1 ");
            ncnn::Mat anchors(6);
            anchors[0] = 10.f;
            anchors[1] = 13.f;
            anchors[2] = 16.f;
